...
class Up(nn.Module):
    def __init__(
        self, in_channels, out_channels, scale_factor=2, norm_cfg=dict(type="BN")
    ):
        super().__init__()
...
class BevEncode(nn.Module):
    def __init__(
        ...
        multiview_learning=False,
        feature_fuse_type="SUM",
            ...
            for i in range(len(num_layer) - 1):
                downsample_conv_list.append(
                    nn.Sequential(
                        ...
                        build_norm_layer(
                            norm_cfg, numC_input * 2 ** (i + 1), postfix=0
                        )[1],
                        nn.ReLU(inplace=True),
                    )
                )
        ...
        self.backbone_output_ids = (
            range(len(num_layer) - 3, len(num_layer))
            ...
            for i in range(len(num_layer)):
                layer = [
                    Bottleneck(
                        ...
                        num_channels[i] // 4,
                        stride=2,
                        downsample=nn.Conv2d(curr_numC, num_channels[i], 3, 2, 1),
                        norm_cfg=norm_cfg,
                    )
                ]
        ...
        if norm_cfg["type"] == "BN":
            self.up2 = nn.Sequential(
                ...
                nn.Conv2d(
                    numC_output * 2, numC_output, kernel_size=3, padding=1, bias=False
                ),
...
class TransformerLiftSplatShootMultiview(BaseModule):
    def __init__(
        ...
        **kwargs
        ...
        print("TransformerLiftSplatShootMultiview")
        ...
        if self.bev_encode:
            if bev_encoder_type == "resnet18":
                self.bev_encode_module = BevEncode(
                    numC_input=self.numC_Trans,
                    numC_output=self.numC_output,
    ...
    def forward_without_bevencoder(self, input):
        ...
        bev_feat_list = [
            bev_feat[:, volume_channel_index[i] : volume_channel_index[i + 1], ...]
