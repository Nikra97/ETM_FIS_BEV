...
class FeatureWarper(object):
    def __init__(self, grid_conf=None, input_dim=None):
        ...
        xs = torch.linspace(
            self.bx[0], self.spatial_extent[0], self.nx[0], dtype=torch.float
        ).view(1, -1)
        ys = torch.linspace(
            self.bx[1], self.spatial_extent[1], self.nx[1], dtype=torch.float
        ).view(-1, 1)

        self.bev_grid = torch.stack(
            (xs.expand(self.nx[1], self.nx[0]), ys.expand(self.nx[1], self.nx[0])),
            dim=2,
        )
    ...
    def warp_features(self, x, flow, mode="nearest"):
        ...
        """
        with record_function("Warp_features"):
            ...
            # [h, w, 3, 1]
            points = torch.cat(
                (self.bev_grid, torch.ones_like(self.bev_grid)[..., :1]), dim=-1
            ).unsqueeze(-1)

            # [b, 1, 1, 2, 3] @ [1, h, w, 3, 1] ==> [b, h, w, 2, 1]
            trans_points = xy_flow.view(-1, 1, 1, 2, 3) @ points.unsqueeze(0).type_as(
                xy_flow
            )
    ...
    def cumulative_warp_features_reverse(
        self, x, flow, mode="nearest", bev_transform=None
    ):
