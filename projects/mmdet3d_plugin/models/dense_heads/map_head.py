...
class BevFeatureSlicer(nn.Module):
    ...
    def __init__(self, grid_conf, map_grid_conf):
        ...
        if grid_conf == map_grid_conf:
            ...
            self.map_grid = torch.stack(
                torch.meshgrid(self.norm_map_x, self.norm_map_y, indexing="xy"), dim=2
            )
...
class MapHead(BaseTaskHead):
    def __init__(
        ...
        **kwargs,
        ...
        if self.binary_cls:
            self.semantic_seg_criterion = BinarySegmentationLoss(pos_weight=pos_weight)
        else:
    ...
    def loss(self, predictions, targets):
        ...
        if self.binary_cls:
            ...
            assert (
                predictions["semantic_seg"].shape[-2:]
