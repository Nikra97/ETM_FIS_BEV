...
class MotionHead(BaseTaskHead):
    def __init__(
        ...
        **kwargs,
        ...
        super(MotionHead, self).__init__(task_dict, in_channels, init_cfg, norm_cfg)
        print("MotionHead")
        ...
        if self.n_future > 0:
            ...
            future_distribution_in_channels = (
                self.in_channels + self.n_future * future_dim
            )
    ...
    def forward(self, bevfeats, targets=None, noise=None):
        ...
        for bevf in bevfeats:
            self.logger.debug(f"BEVfeat: {str(bevf.shape)}")
        if self.training:
            (
        ...
        if self.n_future > 0:
            ...
            self.logger.debug(
                "Temp future_prediction_input shape"
                + str(future_prediction_input.shape)
            )
            ...
            self.logger.debug("Temp flatten_states shape " + str(flatten_states.shape))
