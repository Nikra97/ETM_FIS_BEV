...
class PETRHead(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        x = mlvl_feats[0]
        print(f"PETR_HEAD FORWARD Input: {x.shape =}")
        ...
        print(f"PETR_HEAD FORWARD Input Proj: {x.shape =}")
        x = x.view(batch_size, num_cams, *x.shape[-3:])
        # interpolate masks to have the same spatial shape with x
        masks = F.interpolate(
            masks, size=x.shape[-2:]).to(torch.bool)
        print(f"PETR_HEAD FORWARD Mask: {masks.shape}")
        if self.with_position:
            coords_position_embeding, _ = self.position_embeding(mlvl_feats, img_metas, masks)
            pos_embed = coords_position_embeding
            print(f"PETR_HEAD FORWARD posembed: {pos_embed.shape}")
            if self.with_multiview:
        ...
        print(f"PETR_HEAD FORWARD reference_points: {reference_points.shape}")
        query_embeds = self.query_embedding(pos2posemb3d(reference_points))
        print(f"PETR_HEAD FORWARD query_embeds: {query_embeds.shape}")
        ...
        print(f"PETR_HEAD FORWARD outs_dec: {outs_dec.shape}")
        outputs_classes = []
        outputs_coords = []
        for lvl in range(outs_dec.shape[0]):
            reference = inverse_sigmoid(reference_points.clone())
            print(f"PETR_HEAD FORWARD reference: {reference.shape}")
            assert reference.shape[-1] == 3
            outputs_class = self.cls_branches[lvl](outs_dec[lvl])
            print(f"PETR_HEAD FORWARD outputs_class: {outputs_class.shape}")
            print(f"PETR_HEAD FORWARD outs_dec: {outs_dec[lvl].shape}")
            tmp = self.reg_branches[lvl](outs_dec[lvl])
            print(f"PETR_HEAD FORWARD tmp: {tmp.shape}")
            tmp[..., 0:2] += reference[..., 0:2]
            print(
                f"PETR_HEAD FORWARD tmp ref: {tmp[..., 0:2].shape}, {reference[..., 0:2].shape}")
            tmp[..., 0:2] = tmp[..., 0:2].sigmoid()
            tmp[..., 4:5] += reference[..., 2:3]
            print(
                f"PETR_HEAD FORWARD tmp ref: {tmp[..., 4:5].shape} , {reference[..., 2:3].shape}")
            tmp[..., 4:5] = tmp[..., 4:5].sigmoid()
            print(f"PETR_HEAD FORWARD tmptmp: {tmp.shape}")
            outputs_coord = tmp
        ...
        print(f"PETR_HEAD FORWARD all_cls_scores: {all_cls_scores.shape}")
        print(f"PETR_HEAD FORWARD all_bbox_preds: {all_bbox_preds.shape}")
