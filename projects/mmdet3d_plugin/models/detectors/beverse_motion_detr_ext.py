...
class BEVerse_Motion_DETR(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        super(BEVerse_Motion_DETR, self).__init__(
            pts_voxel_layer,
            pts_voxel_encoder,
            pts_middle_encoder,
            pts_fusion_layer,
    ...
    def aug_test(
        ...
        for i in range(len(img)):
            ...
            for flip_bev in flip_bev_list:
                ...
                # decoder head
                bev_output = self.pts_bbox_head(bev_feat, targets=motion_targets)
                # flip-back outputs
    ...
    def flip_bev_output(self, output, flip_x=False, flip_y=False):
        ...
        for pred_key, pred_val in output[map_key].items():
            output[map_key][pred_key][...] = self.flip_feature(
                pred_val, flip_x=flip_x, flip_y=flip_y
            )
        ...
        for head_id in range(len(output[det_key])):
            for key in output[det_key][0][0]:
                ...
                if key in ["rot", "vel"]:
                    if flip_x:
                        output[det_key][head_id][0][key][:, 0] = -output[det_key][
                            head_id
                        ][0][key][:, 0]

                    if flip_y:
                        output[det_key][head_id][0][key][:, 1] = -output[det_key][
                            head_id
                        ][0][key][:, 1]
                    ...
                    if flip_x:
                        output[det_key][head_id][0][key][:, 0] = (
                            1.0 - output[det_key][head_id][0][key][:, 0]
                        )

                    if flip_y:
                        output[det_key][head_id][0][key][:, 1] = (
                            1.0 - output[det_key][head_id][0][key][:, 1]
                        )
        ...
        for pred_key, pred_val in output[motion_key].items():
            ...
            output[motion_key][pred_key][...] = self.flip_feature(
                pred_val, flip_x=flip_x, flip_y=flip_y
            )
            if pred_key in ["instance_offset", "instance_flow"]:
                if flip_x:
                    output[motion_key][pred_key][:, :, 0] = -output[motion_key][
                        pred_key
                    ][:, :, 0]
                if flip_y:
                    output[motion_key][pred_key][:, :, 1] = -output[motion_key][
                        pred_key
                    ][:, :, 1]
        ...
        for task_key in bev_outputs[0]:
            if task_key != "3dod":
                for pred_key in bev_outputs[0][task_key]:
                    bev_outputs[0][task_key][pred_key] = torch.mean(
                        torch.cat([x[task_key][pred_key] for x in bev_outputs], dim=0),
                        dim=0,
                        keepdim=True,
                    )
                continue
            ...
            for head_id in range(len(bev_outputs[0][task_key])):
                for pred_key in bev_outputs[0][task_key][0][0]:
                    bev_outputs[0][task_key][head_id][0][pred_key] = torch.mean(
                        torch.cat(
                            [x[task_key][head_id][0][pred_key] for x in bev_outputs],
                            dim=0,
                        ),
                        dim=0,
                        keepdim=True,
